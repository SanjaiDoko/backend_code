...
module.exports = function (app, io) {
    let data = { status: 0, response: "Invalid Request" },
      ...
      validator.checkCreateBooking = [
        ...
        check("data.*.headCount").notEmpty().withMessage("Room Number is required field"),
        check("data.*.startsAt").notEmpty().withMessage("Room Number is required field"),
        check("data.*.endsAt").notEmpty().withMessage("Room Number is required field"),
        (req, res, next) => {
          const errors = validationResult(req).array();
          if (errors.length > 0) {
            data.response = errors[0].msg;
