...
async function deleteFilesInFolder(filePath, folderName) {
  ...
  if (shareExists) {
    if (filePath.includes("app") === true) {
      //For Dev, Qa, Uat Environments
      fileHierarchyPath = filePath.split("/fileuploads/").pop();
    } else {
      //Only for Local API Work
      fileHierarchyPath = filePath.split("\\fileuploads\\").pop();
    }
    fileHierarchy = fileHierarchyPath.substring(
      0,
      fileHierarchyPath.indexOf("/")
    );

    if (fileHierarchy === "cfs certificates") {
      directoryName =
        CONFIGJSON.azureFilePath.directory + `/CFS Certificates/${folderName}`;
    }
  ...
  try {
    ...
    if (shareExists) {
      ...
      if (fileHierarchy === "registration") {
        ...
        if (!directoryFolderExists) {
          await directoryFolder.create();
        }

        directoryName =
          CONFIGJSON.azureFilePath.directory + `/Registration/${folderName}`;
      } else if (fileHierarchy === "cfs certificates") {
        fileDirectory =
          CONFIGJSON.azureFilePath.directory + "/CFS Certificates";
        directoryFolder = shareClient.getDirectoryClient(fileDirectory);
        directoryFolderExists = await directoryFolder.exists();
        if (!directoryFolderExists) {
          await directoryFolder.create();
        ...
        directoryName =
          CONFIGJSON.azureFilePath.directory +
          `/CFS Certificates/${folderName}`;
      } else if (fileHierarchy === "origin forwarder") {
        fileDirectory =
          CONFIGJSON.azureFilePath.directory + "/Origin Forwarder";
        directoryFolder = shareClient.getDirectoryClient(fileDirectory);
        directoryFolderExists = await directoryFolder.exists();
        if (!directoryFolderExists) {
          await directoryFolder.create();
        ...
        directoryName =
          CONFIGJSON.azureFilePath.directory + `/Booking/${folderName}`;
        ...
        directoryName =
          CONFIGJSON.azureFilePath.directory + `/Milestone pdf/${folderName}`;
      }
...
const downloadFileAzure = async (folderName, fileToDownload, type) => {
  ...
  try {
    ...
    if (shareExists) {
      directoryName =
        CONFIGJSON.azureFilePath.directory + `/${type}/${folderName}`;
      directoryClient = shareClient.getDirectoryClient(directoryName);
      directoryExists = await directoryClient.exists();
            ...
            if (fileToDownload !== "") {
              if (fileToDownload === item.name) {
                ...
                return (
                  ...
                  Buffer.from(fileDownloadBuffer).toString("base64")
                );
              }
            } else {
              fileName = item.name;
...
function checkMaliciousFile(fileObject, filePathKey) {
  ...
  if (fileObject[filePathKey].includes("base64")) {
    ...
    checkCondition =
      fileObject[filePathKey].endsWith(".pdf") ||
      fileObject[filePathKey].endsWith(".PDF");
  }
